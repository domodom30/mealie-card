...
export class MealieTodayCard extends LitElement {
  ...
  private async loadRecipes(): Promise<void> {
    if (!this.hass || !this.config || this._loading) {
      return;
    }
    ...
    try {
      ...
      let recipes: MealiePlanRecipe[] = mealPlans.filter((item) => item && item.recipe);

      if (this.config.entry_types && this.config.entry_types.length > 0) {
  ...
  private renderRecipeImage(recipe: MealiePlanRecipe) {
    ...
    const imageUrl = getRecipeImageUrl(this.mealieBaseUrl, recipe.recipe.recipe_id, !!recipe.recipe.image);
    if (!imageUrl) {
      return html``;
    }

    const recipeUrl = getRecipeUrl(this.mealieBaseUrl, recipe.recipe.slug, this.config.clickable);
    const imageElement = html`
      <div class="recipe-image-container">
        <img src="${imageUrl}" alt="${recipe.recipe.name}" class="recipe-image" loading="lazy" @error=${this.handleImageError} />
      </div>
    `;
  ...
  private renderRecipeName(recipe: MealiePlanRecipe) {
    const recipeUrl = getRecipeUrl(this.mealieBaseUrl, recipe.recipe.slug, this.config.clickable);
    const nameElement = html`<h3 class="recipe-name">${recipe.recipe.name}</h3>`;
  ...
  private renderRecipeDescription(recipe: MealiePlanRecipe) {
    return recipe.recipe.description ? html`<p class="recipe-description">${recipe.recipe.description}</p>` : html``;
  }

  private renderRecipeTimes(recipe: MealiePlanRecipe) {
    const timeBadges = [
      this.config.show_prep_time && recipe.recipe.prep_time ? this.renderTimeBadge('⏱️', formatTime(recipe.recipe.prep_time)) : null,
      this.config.show_perform_time && recipe.recipe.perform_time ? this.renderTimeBadge('🔥', formatTime(recipe.recipe.perform_time)) : null,
      this.config.show_total_time && recipe.recipe.total_time ? this.renderTimeBadge('⏰', formatTime(recipe.recipe.total_time)) : null
    ].filter(Boolean);
